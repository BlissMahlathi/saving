...
# Define the load functions as lambda functions
load_functions = [
...
for row in load_functions:
    ...
    norm_row = []
    for f in row:
        f_prime = lambda x: 4*x + 3 if '2*x' in str(f) else (6*x + 4 if '3*x' in str(f) else 0)
...
ax.legend()
ax.grid(True)
plt.title("Radial Pattern of Linear Approximations in Structural Engineering")
plt.show()
